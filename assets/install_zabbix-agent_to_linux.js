...
remote.call("sudo rpm -q zabbix-agent");
if(remote.execute("sudo rpm -q zabbix-agent").contains("zabbix-agent-2.0")){
        throw "zabbix-agent is already installed."
}

remote.call("sudo yum install -y openssh-clients");
remote.sendFile("./data/zabbix.repo", "/tmp/", "zabbix.repo");
remote.call("sudo mv /tmp/zabbix.repo /etc/yum.repos.d/");
remote.call("sudo yum install -y zabbix-agent");
...
remote.call("sudo mv /tmp/zabbix_agentd.conf /etc/zabbix/zabbix_agentd.conf");
remote.call("sudo /sbin/chkconfig zabbix-agent on");
remote.call("sudo /etc/init.d/zabbix-agent start");
while(!remote.execute("sudo /etc/init.d/zabbix-agent status").contains("実行中")){
	runtime.sleep(3000);
